...
public ShowMenu(id) {
	...
	formatex(szItem, charsmax(szItem) - 1, "Checkpoint #%d", g_CheckpointCount[id]);
	menu_additem(menu, szItem, "1");

	formatex(szItem, charsmax(szItem) - 1, "GoCheck #%d^n", g_GocheckCount[id]);
	menu_additem(menu, szItem, "2");

	menu_additem(menu, "Reset", "3");

	formatex(szItem, charsmax(szItem) - 1, "\wNoclip %s", g_bNoclipGodmode[id] ? "\y[ON]^n" : "\r[OFF]^n");
...
public MenuHandler(id, menu, item) {
	...
	switch(item) {
		...
		case 2: ResetCheckpoint(id);
		case 3: ToggleNoclipGodmode(id);
	}
...
public CheckpointTeleport(id) {
	...
	new data[CheckpointData];
	ArrayGetArray(g_aPlayerCheckpoints[id], g_CheckpointCount[id] - 1, data);
